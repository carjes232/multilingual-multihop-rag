...
def call_ollama_no_rag_json_final(
    ...
    policy = (
        'Return ONLY {"final": "yes"} or {"final": "no"}.'
        if yn
        else 'Return ONLY {"final": "<short>"} where <short> ≤ 6 words.'
    )
    ...
    trials = [
        ...
        (
            "chat_plain",
            {
                "model": model,
                "messages": [
                    {"role": "system", "content": "Answer only the short final answer. Do NOT include <think>."},
...
def main():
    ...
    ap.add_argument(
        "--write-errors",
        action="store_true",
        help=("Write per-model JSONL of wrong answers + aggregated files " "(+ best-of-two)"),
    )
    args = ap.parse_args()
    ...
    with open(summary_csv, "w", encoding="utf-8", newline="") as sf:
        ...
        for model in requested_models:
            ...
            for q, gold in batch:
                ...
                er = exact_match(pr, gold)
                fr = f1_score(pr, gold)
                rag_em += 1 if er else 0
                rag_f1.append(fr)
                rag_lat.append(msr)

                # NO RAG
                pn, msn = call_ollama_no_rag_json_final(
                    model=model, question=q, temperature=args.norag_temperature, max_tokens=args.norag_max_tokens
                )
                pn_raw = pn
                pn = _shorten_final(pn)
                en = exact_match(pn, gold)
                fn = f1_score(pn, gold)
                nr_em += 1 if en else 0
                nr_f1.append(fn)
                nr_lat.append(msn)
            ...
            zer = sum(1 for g in gains if g == 0)
    ...
    per_model_pngs = [
        ...
        if f.startswith("f1_gain_") and f.endswith(".png")
    ]
    ...
    if args.write_errors:
        outputs.extend(
            [
                os.path.join(args.out_dir, f)
                for f in os.listdir(args.out_dir)
                if f.startswith("errors_") and f.endswith(".jsonl")
            ]
        )
